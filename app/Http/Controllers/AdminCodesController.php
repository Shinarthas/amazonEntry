<?php namespace App\Http\Controllers;
	...
	class AdminCodesController extends \crocodicstudio\crudbooster\controllers\CBController {
        ...
        public function getIndex()
        {
            ...
            $data['logs'] = CodeLog::orderBy("id","desc")->with('code')->limit(40)->get();
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Status','name'=>'status','type'=>'radio','validation'=>'required|boolean','width'=>'col-sm-10','dataenum'=>'1|Enabled;0|Disabled'];
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
	        //Your code here
            $likeRule=new LikeRule();
            if(!$likeRule->passes('code',$postdata['code'])){
                throw new \Exception($likeRule->message(), 422);
            }
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {        
	        //Your code here
            //Your code here
            $likeRule=new LikeRule($id);
            if(!$likeRule->passes('code',$postdata['code'])){
                throw new \Exception($likeRule->message(), 422);
            }
            $code=Code::find($id);
            if ($code->status!=$postdata['status']){
                $codeLog=new CodeLog();
                $codeLog->code_id=$code->id;
                $codeLog->old_status=$code->status;
                $codeLog->new_status=$postdata['status'];
                $codeLog->save();
            }
	    }
